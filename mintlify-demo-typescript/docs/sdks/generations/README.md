...
async function run() {
  const result = await mintlifyDemo.generations.getGeneration({
    id: "gen-1234567890",
  });
...
import { generationsGetGeneration } from "mintlify-demo/funcs/generations-get-generation.js";
...
async function run() {
  const res = await generationsGetGeneration(mintlifyDemo, {
    id: "gen-1234567890",
  });
  if (res.ok) {
